            ...
            <div class="mui-card-content">
                <div class="mui-card-content-inner">
                    ...
                    <p>
                        ...
                        <!-- 2. 由于设计到了父子组件的嵌套，所以，无法直接在goodsinfo 页面中获取到选中商品数量的值 -->
...
export default {
    data(){
        return {
            ...
            selectedCount : 1   //保存用户选中的商品数量，默认 认为用户购买数量是1
        }
    },
    created(){
        this.getLunbotu();
    ...
    methods:{
        getLunbotu() {
            this.$http.get("api/getthumimages/" + this.id).then(result => {
                if(result.body.status === 0){
                    //先循环轮播图的每一项，为item添加img属性，因为轮播图组件中，只认识item.img 不认识item.src
        ...
        enter(el,done) {
            ...
            //小球动画优化思路：
            //1. 先分析导致动画不准确的本质原因，我们把小球最终 位移到的位置，已经局限在了某一分辨率下的 滚动条未滚动的情况下：
            //2. 只要分辨率和测试的时候不一样，或者，滚动有一定的滚动距离后，问题就出现了
            //3. 因此，我们经过分析，得到结论：不能把 位置的横纵坐标 直着写死了，而是应该更加不同情况 动态计算这个坐标值
            //4. 经过分析，得到解题思路：先得到图标的横纵坐标 ，再得到小球的 横纵坐标，然后，让y值求差，x值也求差，得到的结果，就是横纵坐标哟啊位移的距离
